---
title: "Week 2: Multi-Layer Perceptrons & Backpropagation"
...
<div class="lecture-header">
  <h1>Week 2: Multi-Layer Perceptrons &amp; Backpropagation</h1>
</div>
```
...
::: {layout-ncol=2}
![**Linear:** As house size increases, price rises monotonically when other factors remain unchanged; a linear model captures this perfectly.](figures/week02/house_price.png)

![**Non-linear:** doctor visits follow a U-shaped curve over age; no linear model can represent this.](figures/week02/doctor_visit.png)
:::
...
::: {.callout-tip collapse="true" .clarification title="Question: How do W and a interact in the forward pass, and why does that make the derivative nonzero only when k equals i?"}
...
::: {.callout-tip collapse="true" .clarification title="Question: The weight gradient uses the previous layer's activations. What do we use at the first layer, where there is no earlier layer?"}
...
Both factors are pre-specified. $\odot$ is element-wise ([Hadamard](https://en.wikipedia.org/wiki/Hadamard_product_(matrices))) multiplication.

::: {.callout-tip collapse="true" .clarification title="Question: Why is the activation derivative combined with an elementwise product here, when the chain rule uses matrix products elsewhere?"}
...
The chain rule for vectors always multiplies by a [Jacobian matrix](https://en.wikipedia.org/wiki/Jacobian_matrix_and_determinant), the matrix of partial derivatives of a layer's outputs with respect to its inputs. The elementwise (Hadamard) product is that same matrix product in the special case where the Jacobian is a [diagonal matrix](https://en.wikipedia.org/wiki/Diagonal_matrix):
...
$$\begin{aligned}
\frac{\partial \mathcal{L}}{\partial a^m_k} &= \left(\sum_{\ell=1}^{N^{m+1}} \frac{\partial \mathcal{L}}{\partial a^{m+1}_\ell} \cdot W^{m+1}_{\ell k}\right) \\
&\quad \cdot \frac{dh^m_k}{da^m_k}
\end{aligned}$$
...
::: {.callout-tip collapse="true" .clarification title="Question: The recursion needs the gradient from layer m+1. What do we use at the last layer, where there is no next layer?"}
...
$$\begin{aligned}
\frac{\partial \mathcal{L}}{\partial a^m} &= \underbrace{\left({W^{m+1}}^\top \frac{\partial \mathcal{L}}{\partial a^{m+1}}\right)}_{\text{from prev step}} \\
&\quad \odot \underbrace{\frac{dh^m}{da^m}}_{\text{known}}
\end{aligned}$$

**Step 3: Weight gradients** for all $m$:
$$\begin{aligned}
\frac{\partial \mathcal{L}}{\partial W^m} &= \underbrace{\frac{\partial \mathcal{L}}{\partial a^m}}_{\text{step 2}} \\
&\quad \cdot \underbrace{(h^{m-1})^\top}_{\text{forward pass}}
\end{aligned}$$
...
::: {.callout-tip collapse="true" .clarification title="Question: How does this change when we train on a batch of examples instead of one?"}
...
## Worked Example: One Forward and Backward Pass {#sec-worked-example}
...
```{=html}
<div style="overflow-x:auto; margin:1.5rem 0;">
<svg viewBox="0 0 640 300" width="100%" style="max-width:640px; display:block; margin:auto; font-family:'Helvetica Neue', Arial, sans-serif;">
  <text x="320" y="30" text-anchor="middle" font-size="24" fill="#555">Forward values, example 1 (column 1)</text>
  <g stroke="#888" stroke-width="1.2">
    <line x1="112" y1="120" x2="308" y2="120"/><line x1="112" y1="120" x2="308" y2="200"/>
    <line x1="112" y1="200" x2="308" y2="120"/><line x1="112" y1="200" x2="308" y2="200"/>
    <line x1="352" y1="120" x2="508" y2="160"/><line x1="352" y1="200" x2="508" y2="160"/>
  </g>
  <g fill="#C6DEF1" stroke="#5795C7" stroke-width="1.5">
    <circle cx="90" cy="120" r="22"/><circle cx="90" cy="200" r="22"/>
  </g>
  <g fill="#C9E4DE" stroke="#57C7AE" stroke-width="1.5">
    <circle cx="330" cy="120" r="22"/><circle cx="330" cy="200" r="22"/>
  </g>
  <circle cx="530" cy="160" r="22" fill="#F7D9C4" stroke="#C78557" stroke-width="1.5"/>
  <g font-size="16" text-anchor="middle" fill="#205279">
    <text x="90" y="126">1</text><text x="90" y="206">2</text>
  </g>
  <g font-size="16" text-anchor="middle" fill="#207965">
    <text x="330" y="126">3</text><text x="330" y="206">0</text>
  </g>
  <text x="530" y="166" font-size="16" text-anchor="middle" fill="#794520">2</text>
  <g font-size="16" text-anchor="middle" fill="#555">
    <text x="90" y="250">h0 = x(1)</text>
    <text x="330" y="250">h1 = ReLU(a1)</text>
    <text x="530" y="250">h2 = a2</text>
    <text x="330" y="82">a1 = (3, -0.5)</text>
  </g>
</svg>
</div>
```
...
$$\begin{aligned}
\text{contribution of example } s &= \frac{\partial \mathcal{L}}{\partial a^1_{1s}}\frac{\partial a^1_{1s}}{\partial W^1_{21}} \\
&\quad + \frac{\partial \mathcal{L}}{\partial a^1_{2s}}\frac{\partial a^1_{2s}}{\partial W^1_{21}}
\end{aligned}$$
