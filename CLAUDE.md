...
- **Slides / teaching:** `/create-lecture` `/compile-latex` `/deploy` `/deploy-course-notes` `/qa-quarto` `/slide-excellence` `/syllabus` `/teach-from-paper` `/scaffold-exercises` `/course-notes` (`init` / `add` / `revise` / `practice` / `audit`; park or promote before audit)
